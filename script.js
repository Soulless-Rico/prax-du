...
let chanceToBuyMinerSpeedUpgradeForFree = 0.1 // 10%
...
const minerSpeedCostElement = document.getElementById('miner-speed-cost');

let minerSpeed = 1 // (multiplier) default = 1, if 2 then mine speed 2x faster if 0.5x then mining is 2 times slower
let minerSpeedCost = 25

function buyMinerSpeed() {

  if (cookieCount < minerSpeedCost) {return};

  if (minersCount < 1) {return};

  rolledNumber = Math.random();

  if (rolledNumber <= chanceToBuyMinerSpeedUpgradeForFree) {

    minerSpeed += 1;
    minerSpeedCost += minerSpeedCost;
  }
  else {
    cookieCount -= minerSpeedCost;
    minerSpeed += 1;
    minerSpeedCost += minerSpeedCost;
    totalSpentCookies += minerSpeedCost;
  }
  render()
}
...
function render() {
  ...
  minerSpeedCostElement.innerHTML = minerSpeedCost ?? 25;
}
